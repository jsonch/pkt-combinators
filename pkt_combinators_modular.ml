...
module type AtomPipeSig = sig 
  ...
  (* streams, vectors, and pipelines *)
  type 'a stream
  type 'a vector
  
  type ('x, 'y) stream_fcn = 'x -> 'y
  type ('x, 'y) pipeline = unit -> ('x, 'y) stream_fcn

  
  val init_pipeline : ('x, 'y) pipeline -> ('x, 'y) stream_fcn
  (* todo: improve "pipeline" related types and 
     find a way to hide the definition from the user *)

  (* pipeline combinators *)
  val iter : ('s, 'x, 'y) atom -> (unit -> 'x stream -> 'y stream)
  val shard : int -> ('s, 'x, 'y) scheduler -> unit -> ('x stream -> ('y stream) vector)
  val parallel : int -> (unit -> 'x stream -> 'y stream) -> unit -> ('x stream vector -> 'y stream vector)
  val flatten : unit -> 'x stream vector -> 'x stream
  val compose : ('x, 'y) pipeline -> ('y, 'z) pipeline -> ('x, 'z) pipeline
  val (>>>)   : ('x, 'y) pipeline -> ('y, 'z) pipeline -> ('x, 'z) pipeline

  (* atom state constructor helpers *)
  val no_constr : 'a -> 'b option ref
  val create_int : int -> 'a -> int ref
  val create_list : 'a list -> 'b -> 'a list ref

  (* stream constructors and converters *)
  val nil_stream :  'a stream
  val cons_stream : 'a -> 'a stream -> 'a stream
  val list_to_stream : 'a list -> 'a stream
  val stream_to_list : 'a stream -> 'a list

  (* driver functions *)
  val stream_map : ('x stream -> 'y stream) -> 'x list -> 'y list
  val stream_iter : 'x stream -> unit


end

(* module AtomPipe = struct *)

module AtomPipe : AtomPipeSig = struct
  ...
  (* combinators might also return vectors of streams. *)
  type 'a vector = 'a list

  (* pipelines are functions from (stream | stream_vector) -> (stream | stream_vector) *)
  type ('x, 'y) stream_fcn = 'x -> 'y
  type ('x, 'y) pipeline = unit -> ('x, 'y) stream_fcn
  ...
  *)
  ...
  let shard n (scheduler : ('s, 'x, 'y) scheduler)  () : ('x stream -> ('y stream) vector) = 
    (* first, create the state *)
  ...
  let parallel n_pipes (pipe : (unit -> 'x stream -> 'y stream))  () = 
    ...
    fun (stream_vector : 'x stream list) ->
      if (n_pipes <> List.length stream_vector) 
        then failwith "type error: for parallel, number of pipes must match number of input streams";
  ...
  let init_pipeline (p: ('x, 'y) pipeline) : ('x, 'y) stream_fcn = p ();;
  ...
  let stream_map initialized_pipeline list = 
    stream_to_list (initialized_pipeline (list_to_stream list))
  ;;

  (* process an infinite stream *)
  let rec stream_iter stream =
    match stream with
    | Nil -> ()
    | Cons (x, xs) ->
        (* process x *)
        stream_iter (Lazy.force xs)
  ;;

  (* some stream constructors *)
  let nil_stream = Nil;;
  let cons_stream a rest = Cons(a, lazy rest);;
...
let parallel_results = stream_map (parallel_port_knock ()) inputs ;;
let serial_results = stream_map (serial_port_knock ()) inputs;;
...
let rec infinite_input dport = 
  ...
  let rest = (infinite_input (dport + 1)) in
  cons_stream pkt rest
;;
...
let infinite_stream_test () = 
  ...
  let input_stream = infinite_input 0 in
  let init_p = init_pipeline pipeline in
  print_endline ("--- starting infinite input test---");
  stream_iter (init_p input_stream);
  print_endline ("--- finished infinite input test---");
  ()
;;
